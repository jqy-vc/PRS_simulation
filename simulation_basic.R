...
h2=0.5
ei <- rnorm(n, mean = 0, sd = sqrt(var(G)/h2*(1-h2)))
y<-G+ei
print(var(y))
...
# # 添加截距项
# X <- cbind(1, x_matrix)  # 加一列1，表示常数项
# Y <- y

# library(MASS)
# # 使用最小二乘法求解带截距的 beta_hat，包括截距项计算估计值
# beta_hat <- ginv(t(X) %*% X) %*% t(X) %*% Y

# # beta_hat[1] 是截距 alpha，beta_hat[2:(m+1)] 是系数 beta
# print(beta_hat[1])         # 截距项 alpha
# print(beta_hat[2:(m+1)])   # 各 SNP 的回归系数
# beta_hat<-beta_hat[2:(m+1)]
